  ...
  if ymin!=0 or ymax!=0: 
    ...
    if all(isnan(v) for v in ax[0]):
      ...
      print("Warning: unable to make plot with the following data\n"+str(data)+
        "\nand filename\n "+ str(filename)+". Exiting histogram() without drawing plot"+
        "because of nan values as computed histogram values: "+str(ax[0])+".\nThis can sometiems happen with small tilesets.\n")
      return 
...
def tile_pair_RNAduplex_energies(sp,temp_in_C,NA_parameter_set='dna_mathews1999.par'):
    ...
    if 0 < global_thread_pool._processes < len(sp): #_threaded:
        ...
        results = [ global_thread_pool.apply_async(
            RNAduplex_multiple, args=(s, temp_in_C, NA_parameter_set, NEGATE))
            for s in list_of_list_of_seqpair ]
        ...
        e = RNAduplex_multiple(sp, temp_in_C, NA_parameter_set, NEGATE)
...
def tile_pair_NUPACK_mfe_energies(sp,temp_in_C):
    ...
    if _threaded and False: #  threading with vanilla mfe() causes problems due to files overwriting each other  getting en arror so turning threaded off: mfe uses file I/O which does not work with 'threaded'  
        results = [global_thread_pool.apply_async(mfe_binding, args=(p[0], p[1], temp_in_C, negate)) for p in sp]
        e = [result.get() for result in results]
    else:
        e = [ mfe_binding(p[0],p[1],temp_in_C, negate)  for p in sp]
    end = get_time(); print(' duration: %.2f seconds, for %d pairs ' % ((end-start), len(e))); sys.stdout.flush()
    return e
...
def determine_file_type(f):
  ...
  consistent_with_seqs_without_names = 1
  ...
  for line in lines:
    ...
    if not ( seqs_with_no_names_line(line) ): consistent_with_seqs_without_names=0
  ...
  if consistent_with_seq_designer_format_file: 
    ...
    return "IDT format"
  elif consistent_with_seqs_without_names: 
    print("Input file is in a format: DNA sequences, but with no tile names.")
    return "seq with no names format"
  else: 
    print("Input file is in unknown format.")
    return "unknown format"


def no_name_file_format_to_IDT_file_format(input_filename):
  try:
...
def main(files,temp_in_C,domain_analysis=1,strand_analysis=1,tile_pair_check=1,
  ...
  for f in files: 
    ...
    elif input_format == "seq with no names format":
      f = no_name_file_format_to_IDT_file_format(f)
...
#########################################################################################
# Run nupack's pfunc, mfe, binding executables and ViennaRNA's RNAduplex executable
#########################################################################################
...
#   # One can use universal_newlines=True in pytohn2, it seems.
