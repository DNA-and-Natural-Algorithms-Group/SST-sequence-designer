...
#             print '\nnum acg evaluations: {}'.format(evals)
    ...
    def populate_tile_pair_name2energy_most_recent(self, temperature, tile_pair_names_to_check_this_time, start_pos, threaded_tile_pairs):
        # this number is chosen because in timing tests, the overhead of
        ...
        if threaded_tile_pairs and len(seqpairs) >= num_parallel_lists:
            ...
            results = [ global_thread_pool.apply_async(
                sd.RNAduplex_multiple, args=(seqpairs, temperature, NA_parameter_set, NEGATE))
                for seqpairs in list_of_list_of_seqpair ]
            list_of_list_of_energies = [result.get() for result in results]
            energies = itertools.chain.from_iterable(list_of_list_of_energies)
        else:
            energies = sd.RNAduplex_multiple(seqpairs, temperature, NA_parameter_set, NEGATE)
    ...
    def check_tile_pairs_heuristic(self, temperature, num_bad_now, num_bad_opt, threaded_tile_pairs, weight_excess):
        if not hasattr(self, 'tile_names_changed_from_opt'): #  True iff this is the first call to the function check_tile_pairs_heuristic
            #  following will be a dict with keys which are all pairs (i.e. of size 63,190)
            tile_pair_names_to_check_this_time = self.tile_pair_names_to_check
        else:
            tile_pair_names_to_check_this_time = [ (t1name,t2name)
                for (t1name,t2name) in self.tile_pair_names_to_check   #  tile_pair_names_to_check is everyone!
                if (t1name in self.tile_names_changed_from_opt
                or  t2name in self.tile_names_changed_from_opt) ]   #  tile_names_changed_from_opt was set
    ...
    #             print 'total tile pairs to check: {}\nnum parallel lists: {}\nmax length of each list for each parallel call to {}: {}'.format(len(tile_pair_seqs), num_parallel_lists, 'RNAduplex' if not USE_NUPACK else 'pfunc', lengths[0])
                results = [global_thread_pool.apply_async(sd.RNAduplex_multiple, args=(seqpairs, temperature, NA_parameter_set, NEGATE)) for seqpairs in list_of_list_of_seqpair]
                list_of_list_of_energies = [result.get() for result in results]
                energies = itertools.chain.from_iterable(list_of_list_of_energies)
            else:
                energies = sd.RNAduplex_multiple(tile_pair_seqs, temperature, NA_parameter_set, NEGATE)
...
#         if num_bad_opt > 0 and len(new_bad_glues) > num_bad_opt: return new_bad_glues
    ...
    def local_search_end_assign(self, temperature, out_filename, mutually_exclusive_tilename_pairs, seqs_start):
        ...
        while len(bad_glues_opt) > 0:
            ...
            glue = random.choice(bad_glues_opt)#  pick a bad glue
            ec = glue.end_constraint #  get EC for picked bad glue
            old_end = glue.canonical_end() #  get currently assigned seq for picked bad glue

            start=time.time()
            new_end_pos = select_new_end_pos(picked_ends, ec, temperature) #  randomly pick a new sequence
            end=time.time()
...
def eval_lattice_binding_energy(end1,end2,temperature):
    return min(sd.binding(end1+end2,wc(end2)+'TTTTT'+wc(end1),temperature, NEGATE),
               sd.binding(end1+end2,wc(end2)+'AAAAA'+wc(end1),temperature, NEGATE))
...
def eval_colocated_end_pair(end1, end2, temperature):
    return max(sd.hairpin(end1+end2, temperature, NEGATE),
               min(sd.hairpin(end1+'T'*4+end2, temperature, NEGATE),
                   sd.hairpin(end1+'A'*4+end2, temperature, NEGATE)))
...
def select_new_end_pos(picked_ends, ec, temperature, threaded=True):
    ...
    for end_pos in random_iter(range(len(ec.ends))):
        ...
        if ec.domain_indv_sec_struct > 0 and not sd.domain_no_sec_struct(end, temperature, ec.domain_indv_sec_struct, threaded):
            log_bad_end('idv%.1f,%.1f_' % (sd.hairpin(end, temperature, NEGATE), sd.hairpin(sd.wc(end), temperature, NEGATE)))
            continue
...
def _eval_pairs(endpairs, temperature, parallel):
    if parallel:
        results = [global_thread_pool.apply_async(sd.binding, args=(end1, end2, temperature, NEGATE)) for (end1,end2) in endpairs]
        energies = [result.get() for result in results]
    else:
        energies = []
        for end1,end2 in endpairs:
            energies.append(sd.binding(end1, end2, temperature, NEGATE))
    return energies
...
def check_for_viennarna():
    ...
    try:
        sd.RNAduplex_multiple([("ACGT","TGCA")], 53, '', False)
    except:
        ...
        print(sd.RNAduplex_multiple([("ACGT","TGCA")], 53, '', False) )
        sys.exit(-1)
